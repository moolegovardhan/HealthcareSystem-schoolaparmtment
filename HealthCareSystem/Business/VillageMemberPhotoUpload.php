...
$pd = new PatientData();
        ...
        try{
            $patientuniqueid = $pd->generateNewUserSeqNumber('Village','Village',"users");
          ...
          $sql = "INSERT INTO users ( username, password, email, mobile, profession, address, name, middlename, lastname, firstname, dob, gender, city, state, zipcode, "
                  ...
                  . ":age,CURDATE(),'Web','Village','$userid','$patientuniqueid','$officeid','$cardtype',DATE_ADD(CURDATE(), INTERVAL 730 DAY) )";
          //echo $sql;
          ...
          $db = $dbConnection->getConnection(); 
           ...
           $finalAddress =  $address1." ".$address2;
            ...
            $stmt->bindParam("address", $finalAddress);  //  echo "Hello ";
             ...
             $stmt->bindParam("firstname", $name);   // echo "Hello ";
            ...
            $villageSql = "INSERT INTO house_member (memberid, villageid, streetname,housenumber, status, createddate, createdby) 
                            VALUES ('$finalUser', '$officeid', '$streetname', '$housenumber', 'Y', CURDATE(), $userid)";
        //    echo ($villageSql);
            $db1 = $dbConnection->getConnection(); 
           $stmt1 = $db1->prepare($villageSql); 
...
$url = "http://".$_SESSION['host']."/".$_SESSION['rootNode']."/Web/village/villageindex.php?page=registermember";
